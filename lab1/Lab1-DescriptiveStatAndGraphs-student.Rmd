...
DesStat(MHeight,"Descriptive Statistics - Male Weight - James Montebell")
...
2.  Obtain the descriptive statistics (N, min, firstQ, median, thirdQ, max, mean, StDev, var) of "Weight", and present them in a nice table "WeightSummary", whose title is "Descriptive Statistics of Weight - Your Name". Finally, save the table to a word file called "WeightSummary.docx".
    ...
    stats <- function(data, title){ 
      datasummary <- as.data.frame(list(N=length(data),min=min(data),firstQ=quantile(data,0.25),median=median(data),thirdQ=quantile(data,0.75),max=max(data),mean=round(mean(data),3),StDev=round(sd(data),3),Var=round(var(data),3)))
    ...
    stats(Weight, "Descriptive Statistics of Weight - Your Name")
    ```
...
1.  Obtain the descriptive statistics (N, min, firstQ, median, thirdQ, max, mean, StDev, var) of "MHeight", and present them in a nice table "MHeightSummary", whose title is "Descriptive Statistics of Males Height - Your Name". Finally, save the table to a word file called "MHeightSummary.docx".
    ...
    stats <- function(data, title, doc_title){ 
      datasummary <- as.data.frame(list(N=length(data),min=min(data),firstQ=quantile(data,0.25),median=median(data),thirdQ=quantile(data,0.75),max=max(data),mean=round(mean(data),3),StDev=round(sd(data),3),Var=round(var(data),3)))

      datasummary<-flextable(datasummary) 
      datasummary<-set_caption(datasummary, title)
      
      save_as_docx(datasummary, path=paste(doc_title,".docx"))
      datasummary
      
    }

    stats(MHeight, "Descriptive Statistics of Height - Your Name", "MHeightSummary")
    ```
...
3.  .Repeat task 1 for these objects.
    ...
    ```{r}
    stats(FHeight,"Descriptive Statistics of Height - Your Name", "FHeightSummary")

    stats(MWeight,"Descriptive Statistics of Weight - Your Name", "MWeightSummary")

    stats(FWeight,"Descriptive Statistics of Weight - Your Name", "FWeightSummary")
...
HeightCount <- set_caption(HeightCount, "Frequency Table of Height - Your Name")
HeightCount
...
WeightCount <- set_caption(WeightCount, "Frequency Table of Height - Your Name")
WeightCount
save_as_docx(WeightCount,path="HeightCount.docx")
...
SexWeightCount <- set_caption(SexWeightCount, "Frequency Table of Height/Sex - Your Name")
...
2.  What is the mean (average) height and weight of the male and female Statistics Students?
    ...
    ```{r}
    mean(data1[data1$Sex == "Male",]$Height)
    mean(data1[data1$Sex == "Male",]$Weight)
    mean(data1[data1$Sex == "Female",]$Height)
    mean(data1[data1$Sex == "Female",]$Weight)
    ```
...
3.  What are the most frequent heights and weights of the students?
    ...
    ```         
      66 is the most frequent hieght and 120 is the most frequent weight.
    ```

4.  How many students have these heights and weights?

    ```         
    8 students for the height and 120 for the weight.
    ```
...
5.  What are the most frequent heights and weights of the Female students, and how many female students have these heights and weights?
    ...
    ```{r}
    HeightCount <- subset(data1, Sex == 'Female') %>% group_by(Sex, Height) %>%
      summarise(Count=n())

    WeightCount <- subset(data1, Sex == 'Female') %>% group_by(Sex, Weight) %>%
      summarise(Count=n())
    ...
    ```         
    Most frequent female hieghts are 62, 63, 65, 66 and each have 4 females. Most frequent female weight is 125 and has 5 females.
    ```
...
6.  By looking at the histogram what can be said about the heights and weights of the students? Does this agree with the results you found from the frequency tables?
    ...
    ggplot(data1, aes(x=data1$Weight))+
      geom_histogram(binwidth=1,color="black", fill="lightblue")
    ...
    ```         
    It shows that around the median of heights and weights there is a higher count students at that height and weight.
    ```

7.  By looking at the histograms, what can be said about the heights and weights of females and males?

    ```         
    It shows that a lot of males are taller and heavier than the tallest and heaviest female. This agrees with what I found because the average hieghts and weights of females are lower than the average for males.
    ```
