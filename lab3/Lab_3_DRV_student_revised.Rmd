...
# 10 < X <= 18.1
pdunif(18.1,1,30) - pdunif(10,1,30)


# 10<= X <= 18.1
pdunif(18.1,1,30) - pdunif(9,1,30)
...
hist(unifrom_sample, breaks=seq(1,30),xlim=c(1,30), freq = FALSE)
...
```         
With sample size of 1000 it looks closer to the original distribution, compared to the sample size of 100 that is more sporatic.
```
...
```         
The close p approaches 50% from the left side, the distribution is skewed right. When p is around 50% it looks relatively symmetric. Then when p grows away from 50% it skews left.
```
...
Binom_plotter(36,0.35,17,10000)
```

Find $P(X \geq 17.4)$ and $P(X \geq 17.6)$

```{r}
Binom_plotter(36,0.35,17.4,10000)
Binom_plotter(36,0.35,17.6,10000)
...
```         
No difference. The change in X is not significant enough to change probability of it occuring.
```
...
```{r}
 ppois(3,2.2, lower.tail = FALSE)
p1 <- 1-ppois(3,2.2)
p1
p2 <- ppois(5.1, 2.2) - ppois(2.2,2.2)
p2
```
...
p1 <- 1-ppois(3,2.8)
p1
p2 <- ppois(5.1, 2.8) - ppois(2.2,2.8)
p2
0
p1 <- 1-ppois(3,3.2)
p1
p2 <- ppois(5.1, 3.1) - ppois(2.2,3.1)
p2
0
p1 <- 1-ppois(3,3.6)
p1
p2 <- ppois(5.1, 3.6) - ppois(2.2,3.6)
p2
0
p1 <- 1-ppois(3,3.7)
p1
p2 <- ppois(5.1, 4.0) - ppois(2.2,4.0)
p2
```

```         
The distribrution becomes right skewed when lambda is 4.0 for the first distribution and around 3.1 for the second.
```
...
unifrom_sample<-rdunif(100,0,20)
hist(unifrom_sample, breaks=seq(0,20),xlim=c(0,20),freq = FALSE)
for (i in 0:20){
  segments(i,0,i,1/(b-a), lwd=2)
}

unifrom_sample<-rdunif(100,0,20)
hist(data0$Sample_3, breaks=seq(0,20),xlim=c(0,20),freq = FALSE)
for (i in 0:20){
  segments(i,0,i,1/(b-a), lwd=2)
}


Binom_plotter(50,0.6,0, 20)
...
```         
dU(0,20) is related to sample 3, Bin(50, .6) is related to sample1, and Pois(10) is related to sample 2.
```
